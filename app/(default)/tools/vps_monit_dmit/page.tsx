...
export default function VPSMonitorPage() {
  ...
  const linkModifiers = [
    {
      urlPattern: "dmit.io", // URL中包含的域名
      replacePattern: /aff=\d+/g, // 要替换的正则表达式
      replacement: "aff=12065" // 替换成的内容
    },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://i-cf.czl.net/vps_monit_dmit", {
          method: "POST",
          headers: {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>DMIT VPS促销监控</CardTitle>
          <CardDescription>
          实时监控DMIT VPS服务器的促销信息，包括配置、价格和可用区域等详细信息。
          </CardDescription>
        </CardHeader>
        <CardContent>

...
export default function VPSMonitorPage() {
  ...
  const linkModifiers = [
    {
      urlPattern: "bwh81.net", // URL中包含的域名
      replacePattern: /aff=\d+/g, // 要替换的正则表达式
      replacement: "aff=75432", // 替换成的内容
      domainReplacement: true // 需要域名替换
    },
  ];

  // 根据规则修改URL
  const modifyUrl = (originalUrl: string): string => {
    ...
    for (const rule of linkModifiers) {
      if (originalUrl.includes(rule.urlPattern)) {
        ...
        // 替换域名 (如果需要的话)
        if (rule.domainReplacement) {
          modifiedUrl = modifiedUrl.replace("bwh81.net", "bandwagonhost.com");
        }
        
        console.log(`[链接替换] 原始URL: ${originalUrl}`);
        console.log(`[链接替换] 替换后URL: ${modifiedUrl}`);
        break; // 找到第一个匹配规则后停止
      }
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://i-cf.czl.net/vps_monit_bandwagonhost", {
          method: "POST",
          headers: {
        ...
        const processedData = data.content.map(item => ({
          ...item,
          productUrl: modifyUrl(item.productUrl),
          price: formatPrice(item.price)
        }));
        
        setVpsData(processedData);
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>搬瓦工 VPS促销监控</CardTitle>
          <CardDescription>
          实时监控搬瓦工 VPS服务器的促销信息，包括配置、价格和可用区域等详细信息。
          </CardDescription>
        </CardHeader>
        <CardContent>
